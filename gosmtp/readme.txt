...
GoSMTP exposes a set of AI Abilities via the WordPress Abilities API, enabling AI clients, assistants, and MCP (Model Context Protocol) adapters to securely interact with your email sending infrastructure.

=== Free Abilities ===

- Get GoSMTP Settings: Returns global configuration settings such as from email, from name, force flags, return path, and logging toggles. Sensitive credentials are never exposed.
- List Configured Mailers: Lists all configured SMTP connections along with their nicknames, mailer types, and backup connections.
- Send a Test Email: Sends a test email through the configured mailer to verify connection settings and reports the execution result.

=== Pro Abilities ===

- Export Email Logs (CSV): Exports email logs matching specified filters directly as a CSV string format for reporting and analysis.
- List Email Logs: Returns a paginated list of logged emails including status, mailer provider, timestamp, and recipient details.
- Get Email Log Details: Returns full details of a single email log entry, including body content, header info, and delivery response.
- Resend an Email Log: Re-sends a previously logged email directly from the logs, optionally allowing dispatch to a new recipient.
- Get Email Sending Report: Returns an aggregated report including total sent/failed counts, overall success rate, and daily performance breakdown.
...
* [Feature] Support for AI Abilities: Get Settings, List Mailers, and Send Test Email.
